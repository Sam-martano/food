...
# Key milestone points: 1999 start, 2017-2018 peak, and 2021-2023 latest
key_points <- food_trends |>
  filter(year %in% c(1999.5, 2017.5, 2022.5))

ggplot(food_trends, aes(x = year, y = rate, color = food_type)) +
  geom_line(aes(group = food_type), linewidth = 1.1) +
  geom_point(size = 2.5) +
  geom_text(
    data = key_points,
    aes(label = sprintf("%.1f%%", rate)),
    vjust = if_else(key_points$food_type == "Ultra-Processed Food", -1.2, 1.8),
    size = 3.3,
    fontface = "bold",
    show.legend = FALSE
  ) +
  ...
  labs(
    title = "Ultra-Processed vs. Unprocessed Food Consumption",
    subtitle = "NHANES 1999–2023 (Share of Daily Caloric Intake)",
    x = "Survey Period",
    y = "Daily Energy Intake (%)",
    color = "Food Category"
  ) +
